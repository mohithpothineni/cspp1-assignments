...
def getWordScore(word, n):
    ...
    score_ = 0
    for i in word:
        score_ += SCRABBLE_LETTER_VALUES[i]
    score_ *= len(word)
    if n == len(word):
        score_ += 50
    return score_
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print("{}, earned {}.Total: {} points".format(userInput,wordScore, totalScore))
                print()
                # Update the hand
                hand = updateHand(hand, userInput)
    # Game is over (user entered a '.' or ran out of letters), so tell user the
    # total score
    print("Game over.Total Score : {}".format(totalScore))
...
def playGame(wordList):
    ...
    while True:
        ...
        if usr_inp not in ('n', 'r', 'e'):
            ...
            tmp_ = dealHand(HAND_SIZE)
            hand_ = tmp_ 
            playHand(tmp_,wordList,HAND_SIZE)
        elif usr_inp == 'r':
            if hand_ == "":
                print("no previous hand played")
            else:
                playHand(hand_,wordList,HAND_SIZE)
        else:
            print("game exit!!!")
            break
